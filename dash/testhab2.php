...
<head>
    ...
    <style>
        ...
        .habitat-image {
            ...
            height: auto;
            object-fit: cover;
            max-height: 400px; /* Limits the height to avoid excessively long images */
        }
        ...
        .card-deck_hb .card_hb {
            ...
            /* Adjust to ensure 3 cards per row with some margin */
            margin: 10px;
        ...
        @media (max-width: 768px) {
            .card-deck_hb .card_hb {
                ...
                /* Adjust to ensure 2 cards per row with some margin */
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if ($current_habitat != $row['habitat']) {
                    ...
                    if (!empty($row['habitat_image'])) {
                        echo '<div class="text-center"><img src="data:image/jpeg;base64,' . base64_encode($row['habitat_image']) . '" alt="' . htmlspecialchars($row["habitat"]) . ' Habitat" class="img-fluid habitat-image mb-4" /></div>';
                    }
